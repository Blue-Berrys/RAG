...
func initLoggingWithFile() (*os.File, error) {
	...
	// 设置日志同时输出到终端和文件
	// 使用 io.MultiWriter 将日志同时写入终端和文件
...
func demonstrateCompleteRAG(ctx context.Context, queryRouter *router.QueryRouter, llmProvider *llm.ZhipuLLM, vectorRetriever *retrieval.VectorRetriever, embeddingProvider embeddingCfg.Provider, milvusClient *milvus.Client) {
	...
	// 演示查询 - 覆盖所有场景
	queries := []string{
		// 场景 1: 简单查询 - 具体菜品做法（触发 vector=0.3, bm25=0.7）
		"红烧肉怎么做？",
		"西红柿豆腐汤羹怎么做？",

		// 场景 2: 分类查询 - 菜系/类型（简单查询）
		"川菜有哪些特色？",
		"有什么好吃的素食菜？",

		// 场景 3: 图检索场景 - 食材关系（触发 relationship_intensity > 0.6）
		"用鸡蛋能做什么菜？",
		"西红柿和鸡蛋能一起做什么菜？",
		...
		// 场景 5: 高复杂度查询 - 多个实体和逻辑词（触发 vector=0.8）
		"川菜中既有辣味又带有甜味的菜品有哪些，以及它们的食材搭配？",

		// 场景 6: 中等复杂度查询（触发默认权重 vector=0.7, bm25=0.3）
		"湖南菜的传统做法和特色是什么？",

		// 场景 7: 测试缓存命中（重复第一个查询）
		"红烧肉怎么做？", // 第二次查询，应该命中 Redis 缓存
	}
